...
class Worker extends EventEmitter {
  ...
  doTick(compDate, timeout) {
    ...
    if (this.status === workerState.RUNNING
      ...
      log.warn('workerMgmt', '[%s] Timeout! Been running for %i ms with no result; terminating.', this.name, compDate - this.lastStart);
      this.terminate();
    }
    ...
    if ([workerState.ERRORED_WAITING, workerState.FINISHED_WAITING].indexOf(this.status) !== -1
      ...
      log.warn('workerMgmt', '[%s] Timeout! Waiting for process to close for %i ms; no results. Terminating.', this.name, compDate - this.lastEnd);
      this.terminate();
    }
