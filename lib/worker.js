...
class Worker extends EventEmitter {
  ...
  terminate() {
    ...
    this.proc.once('exit', () => {
      ...
      this.errorListener('TIMEOUT', false);
    });
    this.proc.kill();
  ...
  finishListener(retval) {
    log.verbose('workerMgmt', '[%s] Worker done, returned %o. Waiting for forked process to stop...', this.name, retval);
    const watchdogTimeout = setTimeout(() => {
      if (this.status === workerState.RUNNING) {
        log.warn('workerMgmt', '[%s] Finish callback received, but process is still around. Terminating.', this.name);
        this.terminate();
      }
    }, 5000);
    log.silly('workerMgmt', '[%s] Debounced post-finish timeout watchdog', this.name);
    this.proc.once('exit', () => {
      clearTimeout(watchdogTimeout);
      log.verbose('workerMgmt', '[%s] Worker finished successfully.', this.name);
      this.setStatus(workerState.FINISHED);
      this.emit('finish', retval);
    });
  }

  errorListener(error, isStackTrace) {
    if (isStackTrace) {
      const [headline] = error.split('\n');
      this.byline = headline;

      log.warn('workerMgmt', '[%s] FATAL: %s', this.name, headline);
      log.silly('workerMgmt', '[%s] TRACE: %s', this.name, error);
    } else this.byline = `Error: ${error}`;
  ...
  launch() {
    ...
    this.proc.on('message', (msg) => {
      if (msg.type === 'log') this.logListener(msg);
      else if (msg.type === 'finish') this.finishListener(msg.returned);
      else if (msg.type === 'fatal') this.errorListener(msg.stack, true);
    });
    ...
    this.proc.once('exit', (code) => {
      ...
      this.byline = 'Finished successfully';
